...
<div
    ...
    <form class="space-y-6" action="{{ route('superadmin.customers.update', $customer) }}" method="POST">
        @csrf
        @method('PUT')
        <h5 class="text-xl font-medium text-gray-900 dark:text-white">Edit Customer: {{ $customer->name }} (Milik: {{
            $customer->user->name }})</h5>
        <div>
        ...
        <div>
            ...
            <textarea id="address" name="address" rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">{{ old('address', $customer->address) }}</textarea>
        </div>
        <button type="submit"
